  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      const res = await fetch("/api/collections", {
        ...
        method: "POST",
        body: JSON.stringify(values), //biar tau berhasil g nya pake notifikasi dr toast npm
      });
      if (res.ok) {
        setloading(false);
        toast.success("collection created alhamdlillah");
        router.push("/collection");
      }
    } catch (err) {
      console.log("[collections_POST]", err); //biar tau errornya apa
      toast.error("error, coba lg");
    }
  };
